            ...
            if "requiredServices" in service:
                for requiredService in service["requiredServices"]:
                    ...
                    if thisService != None:
                        ...
                        log.write( logtype.ERROR, "can't finish use case as service >" + requiredService + "< ran into the defined timeout time, but is a pre-requisite for the service >" + serviceName + "<")
            else:
                service = create_cf_service(btpUsecase, service)
...
def get_cf_service_deletion_status(btpUsecase, service):
    ...
    p = runShellCommandFlex(btpUsecase, command, logtype.CHECK, None, False)
    result = p.stdout.decode()
    if "FAILED" in result:
        return "deleted"
    else:
