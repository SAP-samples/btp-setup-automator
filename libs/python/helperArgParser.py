...
def setup():
    ...
    if btpSetupAutomatorArguments != None and btpSetupAutomatorArguments != "":
...
def assignArgumentsThroughJsonParameterFile(args):
    if args.parameterfile != None and args.parameterfile != "":
        ...
        for key in myParameters:
            ...
            valueToSet = None
            if valueInParametersFile != None and valueInParametersFile != "":
...
def createDefaultParametersFile(toolParametersFile):
    parser = argparse.ArgumentParser()
    if toolParametersFile != None and toolParametersFile != "":
...
def checkProvidedArguments(btpUsecase):
    ...
    while btpUsecase.globalaccount == None or btpUsecase.globalaccount  == "":
