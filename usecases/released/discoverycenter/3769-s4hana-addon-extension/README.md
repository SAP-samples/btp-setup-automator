...
The current script was designed to setup the account and spin up the application which is used by the Discovery Center Mission, [Extend SAP S/4HANA Business Processes Using SAP Netweaver Add-On for Event Enablement](https://discovery-center.cloud.sap/missiondetail/3730/3769/).
...
The code for the application is taken from the repository [SAP-samples/cloud-extension-ecc](https://github.com/SAP-samples/cloud-extension-ecc-business-process/tree/s4h-addon). During the application deployment other services (such as Authorization and Trust Management Service, SAP HANA Schemas & HDI Containers, etc.) will be additionally installed as specified in its MTA Deployment Descriptor file.
...
Currently the use case is designed to use the free tier service plans and requires a productive SAP BTP account. Also the name and the name of the subaccount is preconfigured to "S4HANAExtension" and US10 as region. In case you need to adapt some of the parameters you can parse them via commandline parameters when you call the script, for example to change the region it would look like this:

```bash
./btpsa -parameterfile 'usecases/released/discoverycenter/3769-s4hana-addon-extension/parameters.json' -globalaccount '<your global account subdomain as shown in the SAP BTP cockpit>' -myemail '<your email address>' -region 'region for your subaccount'
```
