...
The setup of this [mission](https://discovery-center.cloud.sap/protected/index.html#/missiondetail/3384/) can be executed by the [btp-setup-automator](https://github.com/SAP-samples/btp-setup-automator).
...
The current script was designed to setup the account and spin up the application which is used by the Discovery Center Mission, [Extend SAP ERP 6.0 (ECC) business processes on SAP BTP](https://discovery-center.cloud.sap/missiondetail/3338/3384/).

The script will create a subaccount with the necessary entitlements and deploy the Business Partner Validation application in the SAP BTP Cloud Foundry Environment. The following services are created:

* Event Mesh with default plan
* SAP HANA Cloud with hana-free plan

You also will be subscribed to the following applications:

* Event Mesh UI
* SAP Launchpad
* Business Application Studio

The code for the application is taken from the repository [SAP-samples/cloud-extension-ecc](https://github.com/SAP-samples/cloud-extension-ecc-business-process/tree/main). During the application deployment other services (such as Authorization and Trust Management Service, SAP HANA Schemas & HDI Containers, etc.) will be additionally installed as specified in its MTA Deployment Descriptor file.

Moreover the script will deploy a [mock server application](https://github.com/SAP-samples/cloud-extension-ecc-business-process/tree/mock) with the pre-delivered mock data. A new destination will be created to connect the Business Partner Validation application with the mock server.
...
Currently the use case is designed to use the free tier service plans and requires a productive SAP BTP account. Also the name and the name of the subaccount is preconfigured to "ECCExtension" and US10 as region. In case you need to adapt some of the parameters you can parse them via commandline parameters when you call the script, for example to change the region it would look like this:

```bash
./btpsa -parameterfile 'usecases/released/discoverycenter/3384-ecc-extension/parameters.json' -globalaccount '<your global account subdomain as shown in the SAP BTP cockpit>' -myemail '<your email address>' -region 'region for your subaccount'
```
...
Be aware in case you change the region for the use case you need to have a look at the [Available Regions here](https://help.sap.com/products/BTP/65de2977205c403bbc107264b8eccf4b/557ec3adc3174ed4914ec9d6d13487cf.html?locale=en-US&version=Cloud). In order to make sure the the use case is executable you need to check if the [SAP BTP Cloud Foundry Runtime availability](https://discovery-center.cloud.sap/serviceCatalog/cloud-foundry-runtime?region=all&tab=service_plan) and the [SAP HANA Cloud availability](https://discovery-center.cloud.sap/serviceCatalog/sap-hana-cloud?region=all&tab=service_plan) is given.
...
```bash
./btpsa -parameterfile 'usecases/released/discoverycenter/3384-ecc-extension/parameters.json' -globalaccount '<your global account subdomain as shown in the SAP BTP cockpit>' -myemail '<your email address>'
```
